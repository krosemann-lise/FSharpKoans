...
module ``about the stock example`` =
    ...
    let ParsedOpenAndCloseOnDate (input: string) =
        let parse number =
            Double.Parse(number, CultureInfo.InvariantCulture)

        let columns = input.Split(',')
        (columns[0], parse columns[1], parse columns[4])

    [<Koan>]
    let ParsedOpenAndCloseOnDate_ReturnsParsedOpenAndClosePricesOfDate () =
        AssertEquality
            (ParsedOpenAndCloseOnDate "2012-03-30,32.40,32.41,32.04,32.26,31749400,32.26")
            ("2012-03-30", 32.40, 32.26)

    let OpenCloseDifference openAndCloseOnDate =
