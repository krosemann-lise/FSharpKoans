...
module MushroomKingdom =
    ...
    let powerUpWith power character = { character with Power = Some power }
...
module ``about modules`` =
    ...
    let ModulesCanContainFunctions () =
        let superMario =
            MushroomKingdom.Mario |> MushroomKingdom.powerUpWith MushroomKingdom.Mushroom
...
module ``about opened modules`` =
    ...
    [<Koan>]
    let OpenedModulesValuesAndTypes () =
        
        let superMario = { Mario with Power = Some Mushroom }

        AssertEquality superMario.Name "Mario"
        AssertEquality superMario.Occupation "Plumber"
        AssertEquality superMario.Power.Value Mushroom

        let moduleType = Mario.GetType()
